...
-- % : 이 기회의 자리에 0자릿수 이상의 모든 문자가 올 수 있음
...
-- 이름의 세번쨰 자리에 M이 들어가는 직원 사번, 이름 조회
SELECT e.empno
     , e.ename
  FROM emp e
...
-- IS NO NULL : 비교하려 하는 컬럼의 값이 NULL 이 아니면 true(참(
--              비교하려 하는 칼럼의 값이 NULL 일 때 false(거짓)
...
--     OR 연산자으로 사용되므로 혼동하면 안됨!

SELECT 'Hello, SQL' AS greetiog
  FROM dual
; 

SELECT 'Hello, SQL' || ' in Oracle!' AS greetiog
  FROM dual
;

--Hello, SQL in Oracle!

SELECT '안녕하세요~, SQL' || ' 오라클에서 배우고 있어요!' AS greetiog
  FROM dual
;
