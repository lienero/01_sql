...
-- 하지만 특정 행 삭제시 PK 가 아닌 조건으로 삭제하는 것은
-- 주의해야 함.
...
-- 롤백은 성공인 듯 보이지만 
-- TRUNCATE 명령은 싱행과 동시에 자동커밋이 이루어지므로
...
ROLLBACK TO do_update_addr; -- 롤백 세이브포인트를 잘못 잡은 것
-- 롤백 완료.
...
-----------------------------------------------------------
PK_USER	        NORMAL	    CUSTOMER	
PK_DEPT	        NORMAL	    DEPT	
PK_EMP  	    NORMAL	    EMP	
PK_MAIN3	    NORMAL	    MAIN_TABLE3	
UK_NICKNAME3	NORMAL	    MAIN_TABLE3	
PK_MAIN4	    NORMAL	    MAIN_TABLE4	
UK_NICKNAME4	NORMAL	    MAIN_TABLE4	
...
-- 예) MEMBER 테이블의 MEMBER_ID 컬럼에 대해서 인덱스 생성 시도
...
==> PK_MEMBER라는 이름과 다른 IDX_MEMBER_ID 로 생성 시도해도
